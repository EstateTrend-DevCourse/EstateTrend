...
def detail(request, region_id):
    ...
    this_month_trading = RealEstateTrade.objects.filter(trade_year=current_year, trade_month=current_month)
    trading_volume = this_month_trading.count() #이번달 거래량
    print(f"This month's trading volume: {trading_volume} trades") #test code
    # 평균값 구하는 과정
    total_price = this_month_trading.aggregate(total_price=models.Sum('trade_price'))['total_price']
    if trading_volume > 0:
    ...
    if highest_price_housing:
        highest_price_trade = highest_price_housing.real_estate_trade_set.first()
    ...
    lowest_price_housing = RealEstate.objects.filter(region=region).order_by('real_estate_trade_set__trade_price').first()
    if lowest_price_housing:
        lowest_price_trade = lowest_price_housing.real_estate_trade_set.first()
        if lowest_price_trade:
            lowest_price = lowest_price_trade.trade_price
            print(f"The lowest priced housing in {si_do_name} {gu_gun_name} {dong_name}: {lowest_price_housing.estate_name}, Price: {lowest_price}")
        else:
