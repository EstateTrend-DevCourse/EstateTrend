...
sido_coordinate = {'서울특별시':[37.5311, 126.9814],'강원특별자치도': [37.6911, 127.8787],'경기도':[37.6034, 127.1396],
                    '경상북도':[36.3509, 128.6874],'경상남도':[35.8313, 128.6097],'대구광역시':[35.8313, 128.6097],
                    '광주광역시':[35.1502, 126.8565],'대전광역시':[36.3503, 127.3662],'부산광역시':[35.2048, 129.0835],
                    '세종특별자치시':[36.4801, 127.2889],'울산광역시':[35.5664, 129.3190],'인천광역시':[37.4753, 126.6369],
                    '전라남도':[35.0675, 126.9940],'전라북도':[35.8242, 127.1489],'제주특별자치도':[33.4996, 126.5312],
                    '충청남도':[36.4548, 126.7946],'충청북도':[36.8109, 127.7952]}
...
def SigugunMap(sido_name):
    ...
    sigugun_map = folium.Map(location=[sido_coordinate[sido_name][0], sido_coordinate[sido_name][1]], zoom_start=11)
    
    sigugun_boundary = folium.Choropleth(
        sigugun_geo, name = "sigugun-map",
        data=data,
    ...
    folium.GeoJson(
        ...
        #click
        popup=folium.GeoJsonPopup(
            fields=['sgg_nm', 'sgg_cd'],
            aliases=['구이름', '코드번호']
        ),
        tooltip=folium.GeoJsonTooltip(
        fields=['sido_nm', 'sido_cd'],
        aliases=['시이름', '코드번호'] 
        )
    ).add_to(sigugun_map)   
...
def DongMap(sido_name, sigugun_name):
    ...
    dong_map = folium.Map(location=[sido_coordinate[sido_name][0], sido_coordinate[sido_name][1]], zoom_start=11)
    dong_boundary = folium.Choropleth(
        dong_geo, name = "dong-map"
    ).add_to(dong_map)
